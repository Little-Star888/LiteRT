...
flags.DEFINE_string("model", None, "path to the tflite model", short_name="m")
flags.DEFINE_string("output_dir", None, "path to output folder", short_name="o")
flags.DEFINE_string("hostname", "localhost", "hostname for adb", short_name="H")
flags.DEFINE_string("serial", None, "serial for adb", short_name="s")
flags.DEFINE_string("soc_model", None, "SoC Model (e.g. SM8650)")
flags.DEFINE_string("htp_arch", None, "HTP Arch (e.g. V75)")
flags.DEFINE_string("qairt_sdk", None, "Path to qairt sdk folder")

flags.mark_flag_as_required("model")
flags.mark_flag_as_required("output_dir")
flags.mark_flag_as_required("serial")
flags.mark_flag_as_required("soc_model")
flags.mark_flag_as_required("htp_arch")
...
_APPLY_PLUGIN_MAIN = _LITERT_ROOT / "tools" / "apply_plugin_main"
_EXTRACT_BYTECODE = _LITERT_ROOT / "tools" / "extract_bytecode"
...
def _get_ctx_bin_info(ctx_bin_path: str, qairt_sdk: Path) -> dict[str, Any]:
  ...
  Raises:
      ...
      subprocess.CalledProcessError: If the qnn-context-binary-utility command
        fails.
  """
  json_path = Path(FLAGS.output_dir) / "tmp.json"
  subprocess.run(
      [
          qairt_sdk
          / "bin"
...
def _generate_zero_inputs(ctx_bin_info: dict[str, Any], graph_idx: int) -> None:
  ...
  graph_input_path = Path(FLAGS.output_dir) / "inputs" / graph_name
...
def _generate_profiler_output(
  ...
  cmd = f"{adb_cmd} pull {_DEVICE_WORKING_DIR}/output_htp {FLAGS.output_dir}"
  subprocess.run(cmd, check=True, shell=True)
  log_path = Path(FLAGS.output_dir) / "output_htp" / "qnn-profiling-data_0.log"

  # Try to find in output_dir first (where we directed apply_plugin_main to run)
  schematic_path = Path(FLAGS.output_dir) / f"{ctx_bin_name}_schematic.bin"
...
def _generate_ctx_bin(
  ...
  Raises:
      RuntimeError: If the context binary path cannot be extracted from the
        subprocess output.
  """
  tmp_tflite_path = Path(FLAGS.output_dir) / "tmp.tflite"
  ...
  apply_pluin_main_cmd = [
      ...
      "--",
      "--libs=third_party/odml/litert/litert/vendors/qualcomm/compiler",
      "--cmd=apply",
      f"--model={model_path}",
      f"--o={tmp_tflite_path}",
  ...
  logging.debug("Running: %s", " ".join(apply_pluin_main_cmd))
  subprocess.run(apply_pluin_main_cmd, check=True, cwd=cwd, env=env)
...
def main(argv):
  ...
  ctx_bin = _generate_ctx_bin(Path(FLAGS.model), FLAGS.soc_model, qairt_sdk)
  ctx_bin_info = _get_ctx_bin_info(str(ctx_bin), qairt_sdk)
  adb_cmd = _get_adb_cmd(FLAGS.hostname, FLAGS.serial)
  ...
  for index, _ in enumerate(ctx_bin_info["graphs"]):
    ...
    profiling_output_dir = Path(FLAGS.output_dir) / f"qnn_partition_{index}"
  ...
  os.remove(ctx_bin)
  logging.info("Success! Profiling data is in %s", FLAGS.output_dir)
